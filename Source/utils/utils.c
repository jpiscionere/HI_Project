...
void setup_float_bins(const char *fname,float *rmin,float *rmax,int *nbin,float **rupp)
{
	//set up the bins according to the binned data file
	//the form of the data file should be <rlow  rhigh ....>
	const int MAXBUFSIZE=1000;
...
void copy_from_file_to_file(FILE *dest, FILE *source, size_t size)
{
  assert(sizeof(char) == 1);//Must hold true - should disable this check
  int chardata;//fgetc returns an int (cast to an int from unsigned char)
  size_t bytes=0;
  
  while(bytes < size) {
	chardata = fgetc(source);
	assert(chardata != EOF && "Read error while copying bytes from file");
	int nwritten = fputc(chardata, dest);
	assert(nwritten != EOF && "Write error while copying bytes");
	assert(nwritten == chardata && "Write verification failed while copying bytes");
	bytes++;
  }
...
void move_existing_file(const char *fname)
{
  ...
  if (fp != NULL) {
    ...
    my_snprintf(commandstring,len,"%s.bak",fname);
    move_existing_file(commandstring);
    my_snprintf(commandstring,len,"mv %s  %s.bak",fname,fname);
    /* fprintf(stderr,"executing command `%s'\n",commandstring); */
    run_system_call(commandstring);//probably should check for exit status..
  }
}
